...
export default function DashboardPage() {
  ...
  const metricCards: {
    ...
    {
      label: "Tasks Today",
      value: data?.metrics.tasksToday ?? 0,
      Icon: ClipboardList,
    },
    {
      label: "Streak",
      value: `${data?.metrics.streakDays ?? 0} days`,
      Icon: CheckCircle2,
    },
    {
      label: "Overall Readiness",
      value:
        data?.metrics.readinessScore == null
          ? "No score"
          : `${data.metrics.readinessScore}%`,
      Icon: BarChart3,
    },
  ];
  return (
      ...
      <section className="mx-auto flex w-full max-w-7xl flex-col gap-6 px-4 py-5 sm:px-6 sm:py-8 lg:px-8">
        <header className="overflow-hidden rounded-lg border border-[#d8d1c2] bg-white shadow-sm">
          <div className="flex flex-col gap-6 border-b border-[#e7dfd2] px-5 py-5 sm:px-6 lg:flex-row lg:items-start lg:justify-between lg:px-7">
            <div className="max-w-3xl">
              ...
              <h1 className="mt-2 text-3xl font-semibold tracking-tight text-[#17201a]">
                Study dashboard
              </h1>
          ...
          <div className="grid gap-0 sm:grid-cols-3">
            {metricCards.map(({ label, value, Icon }) => (
              <div
                key={label}
                className="flex items-center justify-between border-b border-[#e7dfd2] px-5 py-4 last:border-b-0 sm:border-b-0 sm:border-r sm:last:border-r-0 sm:px-6"
              >
                <div>
              ...
              <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                <div>
                  ...
                  <CardDescription className="mt-2">
                    Task status updates live through Supabase Realtime.
                  </CardDescription>
                </div>
              ...
              {data?.tasks.length ? (
                <div className="divide-y divide-[#eee6da]">
                  {data.tasks.map((task) => (
                    <label
                      key={task.id}
                      className="group grid cursor-pointer grid-cols-[auto_minmax(0,1fr)] gap-3 px-5 py-4 transition hover:bg-[#fbf8f1] sm:grid-cols-[auto_minmax(0,1fr)_auto] sm:items-center sm:px-6"
                    >
                      <input
                        className="mt-1 h-4 w-4 rounded border-[#c8d2cb] accent-[#3b6f6a] sm:mt-0"
                        type="checkbox"
                        checked={task.status === "completed"}
                        onChange={() => toggleTask(task)}
                        aria-label={`Mark ${task.topic ?? task.task ?? "study task"} as ${task.status === "completed" ? "pending" : "completed"}`}
                      />
                      <span className="min-w-0">
                        <span className="block truncate text-sm font-semibold text-[#17201a] group-hover:text-[#2f615c]">
                          {task.topic ?? task.task ?? "Study task"}
                        </span>
                ...
                <div className="px-5 py-12 text-center sm:px-6">
                  ...
                  <p className="mt-1 text-sm text-[#68766f]">
                    Generate a plan to fill today&apos;s queue.
                  </p>
                </div>
              )}
            </CardContent>
          ...
          <aside className="grid gap-4">
            <Card className="overflow-hidden">
              ...
              <CardContent className="space-y-3 p-5">
                {data?.revisionQueue.length ? (
                  data.revisionQueue.map((item) => (
                    <div
                      key={item.id}
                      className="rounded-md border border-[#d8d1c2] bg-[#fbf8f1] px-3 py-3 text-sm"
                    >
                      <p className="line-clamp-3 leading-6 text-[#4f5f57]">
                        {getNoteText(item)}
                      </p>
              ...
              <CardContent className="p-5">
                {data?.analysis?.weak_areas?.length ? (
                  <div className="flex flex-wrap gap-2">
                    {data.analysis.weak_areas.map((area) => (
                      <span
                        key={area}
                        className="rounded-md border border-[#c8d2cb] bg-[#eef8f1] px-2.5 py-1.5 text-xs font-semibold text-[#2f615c]"
                      >
                        {area}
                      </span>
                    ))}
                  ...
                  <p className="rounded-md border border-dashed border-[#c8d2cb] bg-[#fbf8f1] px-3 py-6 text-center text-sm text-[#68766f]">
                    No analysis yet.
                  </p>
                )}
              </CardContent>
        ...
        >
          <Button
            asChild
            variant="outline"
            className="h-12 justify-start bg-white px-4"
          >
            <Link href="/notes">
