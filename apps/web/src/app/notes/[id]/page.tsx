  ...
  useEffect(() => {
    supabase.auth.getSession().then(async ({ data }) => {
      ...
      const response = await fetch(
        `/api/notes/${params.id}?userId=${data.session.user.id}`,
      );
      ...
      <section className="mx-auto w-full max-w-5xl space-y-6">
        <header>
          ...
          <h1 className="mt-2 text-3xl font-semibold">
            {material?.file_name ?? "Material notes"}
          </h1>
        </header>

        <div className="flex flex-wrap gap-2">
          {[
            ["concise", "Concise Notes"],
            ["detailed", "Detailed"],
            ["exam", "Exam-Style"],
          ].map(([value, label]) => (
            <Button
          ...
          {notes.map((note) => {
            ...
            return (
              <Card key={note.id}>
                ...
                <CardContent>
                  <pre className="whitespace-pre-wrap font-sans text-sm leading-6 text-[#4f5f57]">
                    {visible || content}
                  </pre>
                </CardContent>
              </Card>
        ...
        {!notes.length ? (
          <Card>
            <CardContent className="flex items-center gap-3 p-5 text-sm text-[#68766f]">
