      ...
      <section className="mx-auto flex w-full max-w-4xl flex-col gap-6">
        <header>
          ...
          <p className="mt-1 text-sm text-[#68766f]">
            dueCount {Math.max(queue.length - index, 0)}
          </p>
        </header>
        ...
        {!item ? (
          <Card>
            <CardContent className="flex flex-col items-center gap-3 p-10 text-center">
              <CheckCircle2 className="h-10 w-10 text-[#28533a]" />
              <p className="text-lg font-semibold">All due cards reviewed.</p>
              <p className="text-sm text-[#68766f]">
                Your revision queue is clear for now.
              </p>
            </CardContent>
          </Card>
        ) : (
          <>
            <button
              ...
              className="min-h-80 rounded-lg border border-[#d8d1c2] bg-white p-8 text-left shadow-sm transition hover:border-[#3b6f6a]"
            >
              <div className="flex items-center justify-between gap-4">
                <span className="text-sm font-semibold text-[#3b6f6a]">
                  Card {index + 1} of {queue.length}
                </span>
                <RotateCcw className="h-4 w-4 text-[#68766f]" />
              </div>
              <p className="mt-6 whitespace-pre-wrap text-base leading-7 text-[#4f5f57]">
                {isFlipped
                  ? item.content
                  : item.content.split("EXPLANATION:")[0] ||
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  quality updates intervalDays, easeFactor, repetitions, and
                  nextReviewAt.
                </CardDescription>
              </CardHeader>
              <CardContent className="grid gap-3 sm:grid-cols-3">
                {[
                  ...
                  <Button
                    ...
                    onClick={() => review(rating.q)}
                  >
                    quality {rating.q} · {rating.label}
                  </Button>
                ))}
              </CardContent>
            </Card>
            ...
            <Card>
              <CardContent className="grid gap-3 p-5 text-sm text-[#68766f] sm:grid-cols-4">
                <p>intervalDays {item.intervalDays}</p>
                <p>easeFactor {item.easeFactor}</p>
