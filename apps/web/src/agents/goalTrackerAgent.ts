...
function getSubjectName(
  task: PlanTaskRow,
  plansById: Map<string, StudyPlanRow>,
) {
  if (task.subject_name) return task.subject_name;

  const joinedSubject = Array.isArray(task.subjects)
    ? task.subjects[0]
    : task.subjects;
...
function getPlanDates(planIds: string[], plansById: Map<string, StudyPlanRow>) {
  ...
  const totalPlanDays = Math.max(
    1,
    dateDiffDays(earliestCreatedAt, nearestExamDate),
  );
...
export function buildGoalSnapshot(
  ...
  const subjects = Array.from(subjectGroups.entries()).map(
    ([subject, group]) => {
      ...
      return {
        ...
        status: getStatus(
          completionPercentage,
          dates.expectedCoveragePercentage,
        ),
      };
    },
  );
